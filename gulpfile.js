...
function frontEndJS(done) {
  return gulp.src('./assets/js/script-dev.js')
...
gulp.task('scss', function () {
  var postCssOpts = [
    autoprefixer({ overrideBrowserslist: ['last 2 versions', 'Explorer >= 10', 'Android >= 4.1', 'Safari >= 7', 'iOS >= 7'] }),
    mqpacker
  ];
  postCssOpts.push(cssnano);
  return gulp.src('./assets/scss/style-dev.scss')
    .pipe(sourcemaps.init({ loadMaps: true }))
...
gulp.task('html', function () {
  return gulp.src(['./src/**/*.html', '!./src./**/_*.html', '!./src/_*.html'])
    .pipe(fileinclude())
...
gulp.task('build', function () {
  ...
  var json = gulp.src('./assets/data/*.*')
    .pipe(gulp.dest('./public/dist/data/'));
  var downloads = gulp.src('./assets/downloads/*.*')
    .pipe(gulp.dest('./public/dist/downloads/'));
  var css = gulp.src('./public/css/style.css', { allowEmpty: true })
    .pipe(gulp.dest('./public/dist/css/'));

  var js = gulp.src('./public/js/script.js', { allowEmpty: true })
    .pipe(gulp.dest('./public/dist/js/'));
  var js = gulp.src('./assets/js/jplist.min.js', { allowEmpty: true })
    .pipe(gulp.dest('./public/dist/js/'));
  var html = gulp.src('./public/**/*.html')
    .pipe(gulp.dest('./public/'));
  // var myths_fact = gulp.src('./public/**/*.html')
  //   .pipe(gulp.dest('./public/**/'));
  var settings = gulp.src('./site.webmanifest')
    .pipe(gulp.dest('./public/'));
